...
public class Answer {
  public static double[, ] CreateRandomMatrix(int m, int n, int minLimitRandom, int maxLimitRandom) 
  {
    ...
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
             // Генерируем случайное вещественное число в указанном диапазоне
            double randomNumber = (random.NextDouble() * (maxLimitRandom - minLimitRandom)) + minLimitRandom;
     ...
     for (int i = 0; i < matrix.GetLength(0); i++)
        {
            ...
            {
                Console.Write($"{matrix[i, j]:F2} "); // Вывод чисел с двумя знаками после запятой
            }
            Console.WriteLine();
        }
  }
